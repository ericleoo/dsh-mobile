...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function isNarrowViewport(state) {
      ...
      if (typeof state.narrow === "boolean") return state.narrow;
      const width = typeof state.viewportWidth === "number" ? state.viewportWidth
    ...
    function isDrawerOpen(state) {
      return isNarrowViewport(state) && state !== null && state !== void 0 && state.narrowExpanded === true;
    }
    ...
    function isDetailsOpen(state) {
      if (!isNarrowViewport(state) || state === null || state === void 0) return false;
      if (typeof state.rightbarShown === "boolean") return state.rightbarShown;
      return (state.details ?? 0) > 0;
    }
    ...
    function apply(ctx) {
      ...
      function MenuButton({ variant }) {
        ...
        const open = state.narrowExpanded === true;
