/**
 * @ericleoo/dsh-mobile — browser half.
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const PLUGIN_ID = "@ericleoo/dsh-mobile";
    ...
    const css = [
      ...
      "    height: 100%; padding: 16px 16px calc(env(safe-area-inset-bottom, 0px) + 16px); overflow-y: auto;",
    ...
    /** Narrow breakpoint shared with ui-layout's SIDEBAR_AUTO_COLLAPSE (1024). */
    const NARROW_BREAKPOINT = 1024;

    /**
     * Panel slice of a layout store snapshot. Alpha 1 stored
     * `viewportWidth`/`narrowExpanded`/`rightbarShown` at the top level;
     * alpha 2 nests them under `layoutInfo` (next to `panelInfo`) — accept
     * either.
     */
    function panelsOf(state) {
      if (state === null || state === void 0) return null;
      return state.layoutInfo ?? state;
    }

    /**
     * Whether the layout store snapshot is a narrow viewport. Current
     * backends store `viewportWidth` (narrow is derived); older ones stored
     * a `narrow` boolean — accept either.
     */
    function isNarrowViewport(state) {
      const panels = panelsOf(state);
      if (panels === null) return false;
      if (typeof panels.narrow === "boolean") return panels.narrow;
      const width = typeof panels.viewportWidth === "number" ? panels.viewportWidth
        : typeof window !== "undefined" ? window.innerWidth : NARROW_BREAKPOINT;
      return width < NARROW_BREAKPOINT;
    }

    /** Whether the sidebar drawer is open on a narrow viewport. */
    function isDrawerOpen(state) {
      if (!isNarrowViewport(state)) return false;
      const panels = panelsOf(state);
      return panels !== null && panels.narrowExpanded === true;
    }

    /**
     * Whether the details/right panel is open on a narrow viewport.
     * Current backends report `rightbarShown`; older ones reported a numeric
     * `details` count — accept either.
     */
    function isDetailsOpen(state) {
      if (!isNarrowViewport(state)) return false;
      const panels = panelsOf(state);
      if (panels === null) return false;
      if (typeof panels.rightbarShown === "boolean") return panels.rightbarShown;
      return (panels.details ?? 0) > 0;
    }
    ...
    function apply(ctx) {
      ...
      /** Close the details/right panel across backend renames. */
      ...
      function closeMobilePanel(state) {
        ...
        if (isDrawerOpen(state)) layout.toggleSidebar();
        else if (isDetailsOpen(state)) closeDetailsPanel();
      }
      ...
      ctx.effect(() => {
        ...
        return sessions.list.subscribe(() => {
          ...
          if (isDrawerOpen(snapshot)) layout.toggleSidebar();
        });
      ...
      function MenuButton({ variant }) {
        ...
        if (state === null || !isNarrowViewport(state)) return null;
        const open = (panelsOf(state)?.narrowExpanded ?? false) === true;
      ...
      function MobileOverlay({ useSessions }) {
        ...
        const narrow = isNarrowViewport(state);
        const drawer = isDrawerOpen(state);
        const details = isDetailsOpen(state) && hasSession;
